...
static struct {
    ...
    int16_t *prebuffer;           // Pre-generated audio buffer
    unsigned int prebuffer_size;
    unsigned int prebuffer_write_pos;  // Where we've written to device
    uint64_t sample_position;
    int device_ready;
    uint64_t startup_time_us;      // Startup time for sample-accurate playback
    uint64_t samples_generated;    // Cumulative samples we've generated (prevents drift)
} audio_state = {0};
...
static int init_audio() {
    ...
    // Allocate prebuffer (hold ~1000ms of audio ahead to reduce flush frequency and break periodicity)
    audio_state.prebuffer_size = audio_state.sample_rate;  // 1000ms = 48000 samples
    audio_state.prebuffer = malloc(audio_state.prebuffer_size * sizeof(int16_t));
    if (!audio_state.prebuffer) {
        fprintf(stderr, "Failed to allocate prebuffer\n");
        free(audio_state.buffer);
        snd_pcm_close(audio_state.pcm_handle);
        return -1;
    }
    audio_state.prebuffer_write_pos = 0;
    audio_state.samples_generated = 0;
    
    // Capture startup time for sample-accurate playback (prevents drift)
    struct timeval tv;
    gettimeofday(&tv, NULL);
    audio_state.startup_time_us = tv.tv_sec * 1000000LL + tv.tv_usec;
    
    audio_state.device_ready = 1;
    fprintf(stderr, "Audio initialized: %d Hz, 16-bit mono, period=%lu, buffer=%lu, prebuffer=%u samples\n", 
            audio_state.sample_rate, period_size, buffer_size, audio_state.prebuffer_size);
    
    return 0;
}

// Generate audio into the prebuffer
static void generate_audio_samples(int frequency_hz, unsigned int num_samples) {
    for (unsigned int i = 0; i < num_samples; i++) {
        ...
        // 0.4 factor to prevent clipping with headroom
        audio_state.prebuffer[audio_state.prebuffer_write_pos + i] = (int16_t)(sample * 32767.0f * 0.4f);
    }
    audio_state.sample_position += num_samples;
}

// Write pre-generated audio to device
static int flush_audio_to_device() {
    if (audio_state.prebuffer_write_pos == 0) return 0;
    
    snd_pcm_sframes_t frames_out = snd_pcm_writei(audio_state.pcm_handle, audio_state.prebuffer, audio_state.prebuffer_write_pos);
...
// delta_time_seconds: time elapsed since last call (currently unused - using uptime instead)
// Returns: number of samples written to audio device
static int get_sound_bytes(int frequency_hz, float delta_time_seconds) {
    if (!audio_state.device_ready || !audio_state.pcm_handle) {
        return -1;
    }
    
    // Get current time to calculate samples we SHOULD have generated by now
    struct timeval tv;
    gettimeofday(&tv, NULL);
    uint64_t now_us = tv.tv_sec * 1000000LL + tv.tv_usec;
    uint64_t elapsed_us = now_us - audio_state.startup_time_us;
    
    // Calculate cumulative samples that should be generated (sample-accurate)
    // This prevents rounding errors from accumulating frame-to-frame
    uint64_t samples_should_generate = (uint64_t)(((double)elapsed_us / 1000000.0) * audio_state.sample_rate);
    uint64_t frames_to_generate = samples_should_generate - audio_state.samples_generated;
    
    if (frames_to_generate == 0) return 0;
    
    // Cap to prebuffer space
    if (audio_state.prebuffer_write_pos + frames_to_generate > audio_state.prebuffer_size) {
        frames_to_generate = audio_state.prebuffer_size - audio_state.prebuffer_write_pos;
    }
    if (frames_to_generate == 0) {
        // Prebuffer full, flush it first
        flush_audio_to_device();
        frames_to_generate = samples_should_generate - audio_state.samples_generated;
        if (frames_to_generate > audio_state.prebuffer_size) {
            frames_to_generate = audio_state.prebuffer_size;
        }
    }
    
    // Generate into prebuffer
    generate_audio_samples(frequency_hz, (unsigned int)frames_to_generate);
    audio_state.prebuffer_write_pos += frames_to_generate;
    audio_state.samples_generated += frames_to_generate;  // Track cumulative
    
    // Flush after every frame to keep device continuously fed (prevents gaps and silence)
    // Small, frequent flushes are better than large, periodic ones
    flush_audio_to_device();
    
    return (int)frames_to_generate;
}
...
static void cleanup_audio() {
    ...
    if (audio_state.prebuffer) {
        free(audio_state.prebuffer);
        audio_state.prebuffer = NULL;
    }
}
...
static int test_get_sound_bytes(float delta_time_seconds) {
    ...
    // Transition every 60 frames (exactly 1 second at 60 FPS)
    int freq_index = ((frame_count - 1) / 60) % 4;
    int frequencies[] = {440, 550, 660, 880};
